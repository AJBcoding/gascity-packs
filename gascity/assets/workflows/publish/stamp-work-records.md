This is the internal `publish` formula's `stamp-work-records` post-landing
boundary. It runs on the stock ephemeral `gc.run-operator` only after the
publication steps have closed. Direct publication must therefore have already
...
close source work. Every publish override must preserve this separate successor
and its dependency on `open-pr`.
