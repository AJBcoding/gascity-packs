...
approved `gc.build.integration-result.v2` with `integration.outcome=ready`.
...
Legacy v1 results may use the same landing boundary only as explicitly
`not_stampable` evidence; never discover source stores by bead ID.
