...
typed `gc.build.integration-manifest.v1` artifact and immutable source commit
meaning.
...
broad `bd list`, branch-name scans, or session history.

For every source anchor, require `gc.work_base_commit`, `gc.work_commit`, its
absolute authoritative `work_dir`, an absolute validated implementation-summary
path, a fresh `sha256:` summary hash, changed paths from
`git diff --name-only <base> <result>`, and source-anchor dependencies. Keep
`gc.work_commit` as the source-worktree result; never reinterpret it as an
integrated or landed commit. Shared-drain items may name historical commits in
...
`{{artifact_root}}`, using schema `gc.build.integration-manifest.v1`. Set the
