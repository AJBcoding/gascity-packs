...
Before evaluating proof, read `gc.build.code_review_context_path` and use its
`## Integration Candidate` section as the authority for where commands run.
Run `cd "$SCRATCH_WORKTREE"`, verify `pwd -P`, `HEAD == candidate_sha`, and the
recorded tree identity, then resolve relative command paths there. Do not run
evidence commands from the launcher checkout or source worktrees. If candidate
evidence is unusable, write an iterate finding against review setup.
