...
| GC-BF-BR-015 | GC-BF-US-001 | WHEN an implementation formula completes source work, THE formula SHALL preserve source provenance, record `gc.delivery_state=integration_ready`, leave the source record open, and reserve `gc.work_outcome=shipped` for an exact post-landing transition after portable stamping. Workflow/check/drain/session steps SHALL continue to close with `gc.outcome=pass|fail|skipped`. |
...
| GC-BF-004 | `implementation-base` | Virtual targeted implementation contract | Executes one implementation source anchor with prepare-worktree, implement, and the compatibility-named close-source-anchor submission stage while preserving provenance, recording `integration_ready`, leaving source work open, and maintaining item evidence, requirement coverage, and neutral producer metadata. | `implementation-base.formula.toml`; `../tests/test_formula_assets.py` |
| GC-BF-005 | `implementation-item-base` | Virtual targeted shared-drain item contract | Executes exactly one item in a shared single-lane drain while preserving shared-session context, item sequencing, item evidence, requirement coverage, and an open `integration_ready` source record. | `implementation-item-base.formula.toml`; `../tests/test_formula_assets.py` |
...
| GC-BF-035 | `integration-base` | Virtual targetless integration contract | Produces a typed source manifest, assembles one isolated shadow candidate through a providerless on-demand target, and records immutable typed evidence before review without publication authority. | `integration-base.formula.toml`; `../tests/test_formula_assets.py`; `../tests/test_integrate_candidate.py` |

### Default Implementation

| ID | Formula | Type | Required behavior | Evidence |
| --- | --- | --- | --- | --- |
| GC-BF-008 | `build-basic` | Cataloged concrete default | Extends `build-base`, preserves the full stage sequence, declares methodology metadata, uses built-in planning/decomposition/implementation helpers, assembles one typed shadow candidate, writes its canonical implementation summary before review, exposes stable overrides, validates artifacts, and expands review through `build-basic-review`. | `build-basic.formula.toml`; `../tests/test_formula_assets.py`; `../tests/test_formula_assets.py::FormulaAssetTests::test_producer_stages_gate_artifacts_with_bounded_repair` |
...
| GC-BF-036 | `integrate` | Internal stock integration implementation | Extends `integration-base` and uses the stock deterministic scratch-worktree assembler while keeping push, PR, landed, and shipped authority disabled. | `integrate.formula.toml`; `../tests/test_formula_assets.py`; `../tests/test_integrate_candidate.py` |
...
| GC-BF-011 | `do-work` | Targeted implementation item helper | Extends `implementation-base`, prepares one item worktree, implements owned work with the selected implementation target, records exact source provenance plus `integration_ready`, and leaves the source anchor open for integration and landing. | `do-work.formula.toml`; `../tests/test_formula_assets.py` |
| GC-BF-012 | `do-work-item` | Targeted shared-drain item helper | Extends `implementation-item-base`, runs exactly one shared-drain item with the selected implementation target, records an open `integration_ready` source result, and stays internal/single-lane. | `do-work-item.formula.toml`; `../tests/test_formula_assets.py` |
| GC-BF-013 | `same-session-implement` | Targeted internal shared-drain helper | Documents and executes the pack-facing same-session policy by draining through `do-work-item` with exclusive member access, single-lane sequencing, and open `integration_ready` source records. | `same-session-implement.formula.toml`; `../tests/test_formula_assets.py` |
