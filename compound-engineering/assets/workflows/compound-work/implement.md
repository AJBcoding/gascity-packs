...
`close-source-anchor` compatibility step. That step records
`gc.delivery_state=integration_ready` after verifying `gc.work_commit`; it does
not close source work. Never set `gc.work_outcome=shipped` from this branch or
its passing tests.
